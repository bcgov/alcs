...
from application_docs import application_docs
...
#                     with tqdm(
#                         total=length, unit="B", unit_scale=True, desc=filename
#                     ) as pbar2:
#                         s3.upload_fileobj(
#                             file,
#                             ecs_bucket,
#                             f"migrate/application/{application_id}/{document_id}_{filename}",
#                             Callback=lambda bytes_transferred: pbar2.update(
#                                 bytes_transferred
#                             ),
#                         )
#                     pbar.update(1)
...
def planning_docs(starting_planning_document_id, batch, cursor):
    ...
    try:
        with tqdm(
            total=planning_review_count,
            unit="file",
            desc="Uploading planning_review files to S3",
        ) as pbar:
            while True:
                # Fetch the next batch of BLOB data
                data = cursor.fetchmany(batch)
                ...
                for (
                    ...
                    with tqdm(
                        ...
                        s3.upload_fileobj(
                            file,
                            ecs_bucket,
                            f"migrate/planning_review/{planning_review_id}/{document_id}_{filename}",
                            Callback=lambda bytes_transferred: pbar2.update(
                                bytes_transferred
                            ),
                        )
...
def issue_docs(starting_issue_document_id, batch, cursor):
    ...
    try:
        with tqdm(
            total=issue_count, unit="file", desc="Uploading issue files to S3"
        ) as pbar:
            while True:
                # Fetch the next batch of BLOB data
                data = cursor.fetchmany(batch)
                ...
                for (
                    ...
                    with tqdm(
                        ...
                        s3.upload_fileobj(
                            file,
                            ecs_bucket,
                            f"migrate/issue/{issue_id}/{document_id}_{filename}",
                            Callback=lambda bytes_transferred: pbar2.update(
                                bytes_transferred
                            ),
                        )
...
application_docs(starting_document_id, batch_size, cursor, conn, s3)
