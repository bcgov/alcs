...
@inject_conn_pool
def process_planning_review_staff_journal(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    This function is responsible for initializing entries for notifications in staff_journal table in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "planning_review/sql/planning_review_staff_journal_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        last_entry_id = 0
        with open(
            "planning_review/sql/planning_review_staff_journal.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                cursor.execute(
                    f"{submission_sql} WHERE oprn.planning_review_note_id > '{last_entry_id}' ORDER BY oprn.planning_review_note_id;"
                )
                ...
                try:
                    ...
                    last_entry_id = dict(rows[-1])["planning_review_note_id"]
...
@inject_conn_pool
def clean_planning_review_staff_journal(conn=None):
    logger.info("Start staff journal cleaning")
    # Only clean planning_reviews
    with conn.cursor() as cursor:
        cursor.execute(
            f"DELETE FROM alcs.staff_journal asj WHERE asj.audit_created_by = '{OATS_ETL_USER}' AND asj.planning_review_uuid IS NOT NULL"
        )
