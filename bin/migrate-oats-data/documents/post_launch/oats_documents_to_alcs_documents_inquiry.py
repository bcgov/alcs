...
etl_name = "import_inquiry_documents_from_oats"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def import_oats_inquiry_documents(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    ...
    NOTE:
    Before performing document import you need to import Inquiries from oats.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "documents/post_launch/sql/inquiry/oats_documents_to_alcs_documents_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "documents/post_launch/sql/inquiry/oats_documents_to_alcs_documents.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
...
def _compile_insert_query(number_of_rows_to_insert):
    ...
    return f"""
                        ...
                        ON CONFLICT (oats_document_id) DO UPDATE SET 
                            ...
                            oats_issue_id = EXCLUDED.oats_issue_id, 
...
@inject_conn_pool
def document_inquiry_clean(conn=None):
    logger.info("Start inquiry related documents cleaning")
    with conn.cursor() as cursor:
        cursor.execute(
            f"DELETE FROM alcs.document WHERE audit_created_by = '{OATS_ETL_USER}' AND oats_issue_id IS NOT NULL;"
        )
        conn.commit()
