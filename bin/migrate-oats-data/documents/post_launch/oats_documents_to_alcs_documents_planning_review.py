...
def import_oats_pr_documents(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "documents/post_launch/sql/planning_review/oats_documents_to_alcs_documents_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "documents/post_launch/sql/planning_review/oats_documents_to_alcs_documents.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
...
def _compile_insert_query(number_of_rows_to_insert):
    ...
    return f"""
                        ...
                        ON CONFLICT (oats_document_id) DO UPDATE SET 
                            ...
                            oats_planning_review_id = EXCLUDED.oats_planning_review_id, 
...
@inject_conn_pool
def document_pr_clean(conn=None):
