...
etl_name = "process_srw_in_progress_status"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_alcs_srw_in_progress_status(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    This function is responsible for populating In Progress status of Notifications in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "srw/sql/submission/statuses/srw_in_progress_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "srw/sql/submission/statuses/srw_in_progress.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
