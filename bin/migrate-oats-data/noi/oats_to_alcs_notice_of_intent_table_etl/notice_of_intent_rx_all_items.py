...
etl_name = "update_notice_of_intent_date_rx_all_items"
...
def update_notice_of_intent_date_rx_all_items(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    ...
    logger.info("Start update_noi_date_rx_all_items")
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "noi/sql/notice_of_intent_base/notice_of_intent_rx_all_items_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        logger.info(f"Total NOI data to update: {count_total}")
        ...
        with open(
            "noi/sql/notice_of_intent_base/notice_of_intent_rx_all_items.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug(
                        f"Retrieved/updated items count: {records_to_be_updated_count}; total successfully updated notice_of_intents so far {successful_updates_count}; last updated application_id: {last_application_id}"
                    )
...
def _update_fee_fields_records(conn, batch_size, cursor, rows):
    parsed_data_list = _prepare_oats_noi_data(rows)
...
def _prepare_oats_noi_data(row_data_list):
    mapped_data_list = []
