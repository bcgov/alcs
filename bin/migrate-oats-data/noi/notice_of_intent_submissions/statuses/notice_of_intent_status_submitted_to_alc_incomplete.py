...
etl_name = "process_alcs_notice_of_intent_submitted_to_alc_incomplete_status"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_alcs_notice_of_intent_submitted_to_alc_incomplete_status(
    conn=None, batch_size=BATCH_UPLOAD_SIZE
):
    """
    This function is responsible for populating Submitted to ALC status of Notice of Intent in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "noi/sql/notice_of_intent_submission/statuses/submitted_to_alc_incomplete/notice_of_intent_status_submitted_to_alc_incomplete_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "noi/sql/notice_of_intent_submission/statuses/submitted_to_alc_incomplete/notice_of_intent_status_submitted_to_alc_incomplete.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                cursor.execute(
                    f"{application_sql} WHERE oa.alr_application_id > {last_application_id} ORDER BY oa.alr_application_id;"
                )
...
_update_query = """
                    ...
                    WHERE submission_uuid = %(uuid)s and status_type_code = 'SUIN'
"""
