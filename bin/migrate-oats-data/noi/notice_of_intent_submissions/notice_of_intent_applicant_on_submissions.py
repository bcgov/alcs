...
etl_name = "process_notice_of_intent_applicant_on_submissions"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_notice_of_intent_applicant_on_submissions(
    conn=None, batch_size=BATCH_UPLOAD_SIZE
):
    logger.info(f"Start {etl_name}")
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "noi/sql/notice_of_intent_submission/applicant/notice_of_intent_applicant_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "noi/sql/notice_of_intent_submission/applicant/notice_of_intent_applicant.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
