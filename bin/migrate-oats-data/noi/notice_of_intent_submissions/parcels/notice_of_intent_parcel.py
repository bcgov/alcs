...
etl_name = "process_alcs_notice_of_intent_parcels"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_alcs_notice_of_intent_parcels(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    Imports notice of intent parcel fields from oats to alcs.notice_of_intent_parcel.
    """

    logger.info(f"Start {etl_name}")
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "noi/sql/notice_of_intent_submission/parcels/notice_of_intent_parcels_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        logger.info(f" Total Notice of Intents parcels to update: {count_total}")
        ...
        with open(
            "noi/sql/notice_of_intent_submission/parcels/notice_of_intent_parcels.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                cursor.execute(
                    f"{application_sql} WHERE decision_date_for_nois.alr_application_id > {last_subject_property_id} ORDER BY decision_date_for_nois.alr_application_id;"
                )
                ...
                try:
                    ...
                    last_subject_property_id = dict(rows[-1])["alr_application_id"]

                    logger.debug(
                        f"retrieved/updated items count: {records_to_be_updated_count}; total successfully updated notice of intents so far {successful_updates_count}; last updated application_id: {last_subject_property_id}"
                    )
