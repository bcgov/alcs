...
WHERE (
    ...
    OR (
        soil_project_duration_amount != soil_to_r.project_duration
        AND soil_to_r.project_duration != NULL
    )
    OR (
        soil_project_duration_unit != 'month'
        AND soil_to_r.project_duration != NULL
    )
...
SELECT *
FROM soil_to_place AS soil_to_p
    JOIN alcs.notice_of_intent_submission nois ON nois.file_number = soil_to_p.alr_application_id::TEXT
WHERE (
        soil_to_place_area != soil_to_p.volume
        AND soil_to_p.volume != NULL
    )
    OR (
        soil_to_place_area != soil_to_p.project_area
        AND soil_to_p.project_area != NULL
    )
    OR (
        soil_to_place_maximum_depth != soil_to_p."depth"
        AND soil_to_p."depth" != NULL
    )
    OR (
        soil_to_place_average_depth != soil_to_p."depth"
        AND soil_to_p."depth" != NULL
    )
    OR (
        soil_project_duration_amount != soil_to_p.project_duration
        AND soil_to_p.project_duration != NULL
    )
    OR (
        soil_project_duration_unit != 'month'
        AND soil_to_p.project_duration != NULL
