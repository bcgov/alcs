WITH -- Step 1: filter out multiple component NOIs
noi_grouped AS (
...
noi_lookup AS (
    SELECT DISTINCT oaap.alr_application_id AS application_id,
        string_agg (DISTINCT oo.organization_name, ', ') FILTER (
            WHERE oo.organization_name IS NOT NULL
        ) AS orgs,
        string_agg (
            DISTINCT concat (op.first_name || ' ' || op.last_name),
            ', '
        ) FILTER (
            WHERE op.last_name IS NOT NULL
    ...
    WHERE oaap.alr_appl_role_code IN ('APPL', 'AGENT')
    GROUP BY oaap.alr_application_id
),
