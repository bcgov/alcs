...
etl_name = "init_inquiry_parcels"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def init_inquiry_parcels(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    logger.info(f"Start {etl_name}")

    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "inquiry/sql/parcels/inquiry_parcel_insert_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        logger.info(f"Total Inquiry Parcels to insert: {count_total}")
        ...
        with open(
            "inquiry/sql/parcels/inquiry_parcel_insert.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            inquiry_sql = sql_file.read()
            while True:
                cursor.execute(
                    f"{inquiry_sql} WHERE osp.subject_property_id > {last_subject_property_id} ORDER BY osp.subject_property_id;"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"inserted items count: {records_to_be_inserted_count}; total successfully inserted inquiry parcels so far {successful_inserts_count}; last inserted subject_property_id: {last_subject_property_id}"
                    )
...
def _insert_records(conn, cursor, rows, insert_index):
    ...
    if number_of_rows_to_insert > 0:
        insert_query = _compile_inquiry_insert_query(number_of_rows_to_insert)
...
def _map_data(row, insert_index):
    return {
        ...
        "civic_address": _map_address(row),
...
def _map_address(data):
    addr = data.get("civic_address", "")
    if addr:
        return addr
    else:
        return "No data found in OATS"


def _compile_inquiry_insert_query(number_of_rows_to_insert):
...
@inject_conn_pool
def clean_inquiry_parcels(conn=None):
    ...
    conn.commit()
    logger.info("Done inquiry parcel cleaning")
