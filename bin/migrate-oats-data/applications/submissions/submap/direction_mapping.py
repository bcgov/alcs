from common import AlcsAdjacentLandUseType
...
def map_direction_values(data, direction_data):
    ...
    data["east_land_use_type_description"] = direction_data.get(data[app_id], {}).get(
        "east_description", no_data
    )
    data["east_land_use_type"] = direction_data.get(data[app_id], {}).get(
        "east_type_code", None
    )
    data["west_land_use_type_description"] = direction_data.get(data[app_id], {}).get(
        "west_description", no_data
    )
    data["west_land_use_type"] = direction_data.get(data[app_id], {}).get(
        "west_type_code", None
    )
    data["north_land_use_type_description"] = direction_data.get(data[app_id], {}).get(
        "north_description", no_data
    )
    data["north_land_use_type"] = direction_data.get(data[app_id], {}).get(
        "north_type_code", None
    )
    data["south_land_use_type_description"] = direction_data.get(data[app_id], {}).get(
        "south_description", no_data
    )
    data["south_land_use_type"] = direction_data.get(data[app_id], {}).get(
        "south_type_code", None
    )
    return data
...
def create_direction_dict(adj_rows):
    ...
    for row in adj_rows:
        ...
        updated_type_code = row[type_code]
        if updated_type_code:
            updated_type_code = AlcsAdjacentLandUseType[updated_type_code].value

        if application_id in direction_dict:
            if row[compass] == "EAST":
                direction_dict[application_id]["east_description"] = row[description]
                direction_dict[application_id]["east_type_code"] = updated_type_code
            if row[compass] == "WEST":
                direction_dict[application_id]["west_description"] = row[description]
                direction_dict[application_id]["west_type_code"] = updated_type_code
            if row[compass] == "NORTH":
                direction_dict[application_id]["north_description"] = row[description]
                direction_dict[application_id]["north_type_code"] = updated_type_code
            if row[compass] == "SOUTH":
                direction_dict[application_id]["south_description"] = row[description]
                direction_dict[application_id]["south_type_code"] = updated_type_code
        else:
