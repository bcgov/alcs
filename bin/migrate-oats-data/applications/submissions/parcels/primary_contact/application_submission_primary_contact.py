...
etl_name = "link_application_primary_contacts"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def link_application_primary_contacts(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    logger.info(f"Start {etl_name}")
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "applications/submissions/sql/parcels/primary_contact/application_submission_primary_contact_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "applications/submissions/sql/parcels/primary_contact/application_submission_primary_contact.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                cursor.execute(
                    f"{application_sql} WHERE apps.file_number::bigint > '{last_file_number}' ORDER BY apps.file_number::bigint;"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"retrieved/updated items count: {records_to_be_updated_count}; total successfully set application primary contacts so far {successful_updates_count}; last set {last_file_number}"
                    )
...
_update_query = """
                    ...
                    WHERE
                        alcs.application_submission.file_number = %(file_number)s::TEXT
"""
