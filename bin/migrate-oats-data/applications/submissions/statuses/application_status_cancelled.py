...
etl_name = "process_alcs_application_cancelled_status"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_alcs_application_cancelled_status(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    This function is responsible for populating Cancelled status of Applications in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "applications/submissions/sql/statuses/application_cancelled_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "applications/submissions/sql/statuses/application_cancelled.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
