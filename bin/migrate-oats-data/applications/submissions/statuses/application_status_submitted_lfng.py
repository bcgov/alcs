...
etl_name = "process_alcs_application_submitted_lfng_status"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def process_alcs_application_submitted_lfng_status(
    conn=None, batch_size=BATCH_UPLOAD_SIZE
):
    """
    This function is responsible for populating Submitted to LFN/G status of Applications in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "applications/submissions/sql/statuses/application_submitted_to_lfng_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        with open(
            "applications/submissions/sql/statuses/application_submitted_to_lfng.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            ...
            while True:
                cursor.execute(
                    f"{application_sql} AND oats_sub_lg.alr_application_id > {last_application_id} ORDER BY oats_sub_lg.alr_application_id;"
                )
...
_update_query = """
                    ...
                    WHERE submission_uuid = %(uuid)s and status_type_code = 'SUBG'
"""
