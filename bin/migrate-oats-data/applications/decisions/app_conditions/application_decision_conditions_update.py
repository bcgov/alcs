...
etl_name = "update_application_conditions"
logger = setup_and_get_logger(etl_name)


@inject_conn_pool
def update_application_conditions(conn=None, batch_size=BATCH_UPLOAD_SIZE):
    """
    This function is responsible for updating existing the application_decision_condition in ALCS.
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        with open(
            "applications/decisions/sql/conditions/application_decision_conditions_update_count.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
        ...
        logger.info(f"Total Application Conditions data to updated: {count_total}")
        ...
        with open(
            "applications/decisions/sql/conditions/application_decision_conditions_update.sql",
            "r",
            encoding="utf-8",
        ) as sql_file:
            query = sql_file.read()
            while True:
                ...
                try:
                    ...
                    _update_application_conditions(conn, batch_size, cursor, rows)
...
def _update_application_conditions(conn, batch_size, cursor, rows):
...
def _get_update_query():
    query = f"""
                 UPDATE alcs.application_decision_condition
                    SET security_amount = %(security_amt)s
                    WHERE alcs.application_decision_condition."uuid" = %(condition_uuid)s;
    """
    return query
