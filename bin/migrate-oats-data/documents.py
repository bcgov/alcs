...
def compile_document_insert_query(number_of_rows_to_insert):
    ...
    return f"""
        INSERT INTO alcs."document" (oats_document_id, file_name, oats_application_id, "source", 
                                    audit_created_by, file_key, mime_type) 
...
def process_application_documents(conn=None, batch_size=10000):
    ...
    with conn.cursor() as cursor:
        with open(
            "sql/documents/app_doc_total_count.sql", "r", encoding="utf-8"
        ) as sql_file:
        ...
        with open("sql/documents/app_doc.sql", "r", encoding="utf-8") as sql_file:
