...
import { type ParcelDetails, type ParcelOwnerDetails } from '../parcels-page';
...
export class ParcelsSection {
  ...
  async expectParcelOwners(parcelNumber: number, owners: ParcelOwnerDetails[]) {
    ...
    for (const owner of owners) {
      ...
      await expect(phoneNumberTexts.filter({ hasText: owner.phoneNumber })).toBeVisible();
  ...
  confirmationText(parcelNumber: number): Locator {
    return this.page.getByTestId(`parcel-${parcelNumber}-is-confirmed-by-applicant`);
  }
