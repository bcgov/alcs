...
export class ViewNoticeOfIntentSubmissionComponent implements OnInit, OnDestroy {
  ...
  $parcels = new BehaviorSubject<NoticeOfIntentParcelDto[]>([]);
      ...
      if (fileId) {
        this.loadSubmission(fileId).then(() => {
          if (this.submission?.uuid) {
            this.loadParcel(this.submission.uuid);
          }
        });
  ...
  async loadParcel(submissionUuid: string) {
    const parcels = await this.noiParcelService.fetchBySubmissionUuid(submissionUuid);
    if (parcels) {
