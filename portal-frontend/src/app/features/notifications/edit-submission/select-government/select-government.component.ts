...
export class SelectGovernmentComponent extends StepComponent implements OnInit, OnDestroy {
  currentStep = EditNotificationSteps.Government;
  ...
  constructor(private codeService: CodeService, private notificationSubmissionService: NotificationSubmissionService) {
    super();
  }
  ...
  ngOnInit(): void {
    ...
    this.$notificationSubmission.pipe(takeUntil(this.$destroy)).subscribe((submission) => {
      if (submission) {
        this.selectGovernmentUuid = submission.localGovernmentUuid;
        this.fileId = submission.fileNumber;
        this.submissionUuid = submission.uuid;
        this.populateLocalGovernment(submission.localGovernmentUuid);
      }
    });
  ...
  private async save() {
    if (this.isDirty) {
      ...
      if (localGovernmentName) {
        ...
        if (localGovernment) {
          const res = await this.notificationSubmissionService.updatePending(this.submissionUuid, {
            localGovernmentUuid: localGovernment.uuid,
          });
          this.$notificationSubmission.next(res);
        }
      }
      this.isDirty = false;
    }
