...
describe('NotificationParcelService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(NotificationParcelService);
  });
  ...
  it('should make a get request for loading parcels', async () => {
    ...
    expect(mockHttpClient.get.mock.calls[0][0]).toContain('notification-parcel');
  });
  ...
  it('should make a post request for create', async () => {
    ...
    expect(mockHttpClient.post.mock.calls[0][0]).toContain('notification-parcel');
  });
  ...
  it('should make a put request for update', async () => {
    ...
    await service.update([{ uuid: mockUuid }] as NotificationParcelUpdateDto[]);

    expect(mockHttpClient.put).toHaveBeenCalledTimes(1);
    expect(mockToastService.showSuccessToast).toHaveBeenCalledTimes(1);
    expect(mockHttpClient.put.mock.calls[0][0]).toContain('notification-parcel');
  });

  it('should show an error toast if updating a parcel fails', async () => {
    mockHttpClient.put.mockReturnValue(throwError(() => ({})));

    await service.update([{}] as NotificationParcelUpdateDto[]);
