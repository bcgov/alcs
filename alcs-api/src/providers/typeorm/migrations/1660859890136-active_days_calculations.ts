...
export class activeDaysCalculations1660859890136 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE OR REPLACE FUNCTION get_weekday_count (start_date DATE, end_date DATE)
          RETURNS int
          LANGUAGE plpgsql
          AS $$
      DECLARE
    ...
    await queryRunner.query(`
      CREATE OR REPLACE FUNCTION get_holiday_count (start_date DATE, end_date DATE)
          RETURNS int
          LANGUAGE plpgsql
          AS $$
      DECLARE
    ...
    await queryRunner.query(`
      ...
      FROM ( --Load each pause for application with each having its own count of weekdays and holidays
          SELECT
              ...
              start_date,
              end_date,
              get_weekday_count (start_date::DATE,
                  COALESCE(end_date, override.now()::DATE)) AS pause_weekdays,
              get_holiday_count (start_date::DATE,
                  COALESCE(end_date, override.now()::DATE)) AS pause_holidays
          FROM (
              SELECT
                  application_uuid,
    ...
    await queryRunner.query(`
      ...
      SELECT
          uuid,
          get_weekday_count (created_at::DATE,
              override.now()::DATE) AS active_days_no_holiday,
          get_holiday_count (created_at::DATE,
              override.now()::DATE) AS active_holidays
      FROM
          application
